...
function Coffee(size, isDecaf, qtyCream, qtySugar){
  this.size = size;
  //this.isDecaf = isDecaf;  // these are properties
  //create a condition instead writing true or false for "isdecaf"
  // using ternary, if this variable is true do this or else (:) do this if false
  this.decaf = (this.isDecaf == true ? "decafeinated" : "caffeinated");
  this.qtyCream = qtyCream;
  this.qtySugar = qtySugar;
  this.description = function(){
    output.textContent = `A ${this.size} ${isDecaf} coffee with ${this.qtyCream} creamers and ${this.qtySugar} teaspoons of sugar`;
  }
}
/* STEP 2: Instatiate a coffee based on the above constructor function */
let isaacsCoffee = new Coffee("large", false, 2, 1) //Instatiate a new coffee object using the new keyword
...
let megansCoffee = Object.create(isaacsCoffee);
//we can alter from 2 to 1
megansCoffee.qtyCream = 1;
//alter from false to true
...
//megansCoffee.__proto__ in the console
//megansCoffee.__proto__.__proto__ in the console
...
//might be helpful if you don't have direct access to the codebase
...
//function name is "thanks"
// dunno what prototype means though
Coffee.prototype.thanks = function() {
  output.textContent = `thanks for a ${this.size} coffee! `;
} // added more abilities, they now have the abilities to thanks.
// in the console do "lukesCoffee.thanks()", "isaacsCoffee.thanks()", "lukesCoffee.thanks()"
