...
HPYABzxDNW46wT2qifXaMIlbIrNh05qlH0tkvmNg
...
http://192.168.1.41/api/HPYABzxDNW46wT2qifXaMIlbIrNh05qlH0tkvmNg/lights/1
...
const user = "HPYABzxDNW46wT2qifXaMIlbIrNh05qlH0tkvmNg";
...
let method = "put"; //put is update, it updates the value
...
hueSlider.addEventListener("change", function() {
	...
	let commands = '{ "hue" : ' + newHue + ', "sat" : 254, "bri" : 214, "on" : true }';
...
// we make async because we want to run other codes, unshackle from the linear aspect
async function updateLight(commands){// <- command is where we throw the json
	// STEP 8a: Use the fetch() method
	let response = await fetch(url, { //<- because it will take bit of time
    method: method, 
    mode: "cors", //what is cors? Cors is something called cross origin resourse sharing, whitelist
    body: commands
