...
let homeWorkDone = null; //null is no value´
let breakDuration = 0;
if (homeWorkDone === false){
  breakDuration = 10; //10 min break
}else{
...
weatherButton.addEventListener("click", setWeather); //<- you dont need to () because we are referrencing not invoking
...
function setWeather() {
  ...
  if (choice === "sunny"){
    ...
    // | is single pipe or, || = short circuit or, checking if x or y is true
